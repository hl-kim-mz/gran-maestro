...
**인자**: `{프롬프트} [--prompt-file {경로}] [--files {패턴}] [--trace {REQ/TASK/label}]`
...
Maestro 오케스트레이션 스킬을 활성화하고 `.gran-maestro/mode.json`을 `active: true`로 업데이트합니다. `/mst:request`를 호출하면 자동 부트스트래핑이 포함되어 있어 `/mst:on`을 별도로 호출할 필요가 없습니다.
...
**한 줄 설명**: Chrome Extension(UI Picker)을 설치합니다. chrome://extensions 페이지 오픈 + Extension 경로 클립보드 복사 + 연결 확인.

**인자**: `[--skip-open]`

#### 목적

Gran Maestro Chrome Extension(UI Picker)을 Chrome에 로드하는 설치 절차를 안내합니다. `chrome://extensions` 페이지를 자동으로 열고, Extension 경로(`{PLUGIN_ROOT}/extension/`)를 클립보드에 복사하며 Dashboard 서버와의 연결 상태를 확인합니다. 독립 유틸리티 스킬로 config.json 참조 없이 동작합니다.
