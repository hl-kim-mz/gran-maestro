...
> **대시보드 URL 해석**: 결과 표시 전에 "Step C-2: 대시보드 URL 구성"과 동일한 절차로 `{DASHBOARD_BASE_URL}`을 먼저 구성한다. 단, `--redesign`은 DES 채번을 건너뛰므로 `{DES-NNN}`이 없다. 활성 REQ/PLN에 `linked_designs`가 존재하면 해당 DES-NNN을 사용하고, 없으면 대시보드 링크를 생략한다.
