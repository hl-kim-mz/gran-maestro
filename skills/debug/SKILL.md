...
> **Claude 모델 결정**: config.resolved.json의 `models.claude` 값 사용 (미설정 시 `"sonnet"` 폴백).
...
- `provider: "codex"`:
  ...
  Bash(
    ...
    command: "codex exec --full-auto -m {config.models.codex.default} -C $(pwd) \"$(cat {absolute_path}/prompts/{investigatorKey}-prompt.md)\" > {absolute_path}/finding-{investigatorKey}.md 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/finding-{investigatorKey}.md; exit $EC"
  )
  ```
- `provider: "gemini"`:
  ```
  Bash(
    run_in_background: true,
    command: "gemini -p \"$(cat {absolute_path}/prompts/{investigatorKey}-prompt.md)\" --model {config.models.gemini.default} --approval-mode yolo > {absolute_path}/finding-{investigatorKey}.md 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/finding-{investigatorKey}.md; exit $EC"
  )
  ```
- `provider: "claude"`:
  ```
