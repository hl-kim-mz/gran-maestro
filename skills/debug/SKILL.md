...
- `provider: "codex"`:
  ...
  Bash(
    ...
    command: "codex exec --full-auto -C $(pwd) \"$(cat {absolute_path}/prompts/{investigatorKey}-prompt.md)\" > {absolute_path}/finding-{investigatorKey}.md 2>&1; echo 'EXIT_CODE:'$? >> {absolute_path}/finding-{investigatorKey}.md"
  )
  ```
- `provider: "gemini"`:
  ```
  Bash(
    run_in_background: true,
    command: "gemini -p \"$(cat {absolute_path}/prompts/{investigatorKey}-prompt.md)\" --model {config.models.gemini.default} --approval-mode yolo > {absolute_path}/finding-{investigatorKey}.md 2>&1; echo 'EXIT_CODE:'$? >> {absolute_path}/finding-{investigatorKey}.md"
  )
  ```
- `provider: "claude"`:
  ```
