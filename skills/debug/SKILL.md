...
- `provider: "codex"`:
  ...
  Bash(
    ...
    command: "codex exec --full-auto -C $(pwd) \"$(cat {absolute_path}/prompts/{investigatorKey}-prompt.md)\" > {absolute_path}/finding-{investigatorKey}.md 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/finding-{investigatorKey}.md; exit $EC"
  )
  ```
- `provider: "gemini"`:
  ```
