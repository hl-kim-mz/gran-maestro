...
Read(`.gran-maestro/config.resolved.json`) → `workflow.default_agent` 추출 → DEFAULT_AGENT 변수 보관.
...
4. PM Conductor 역할로 Phase 1 분석 수행 (`agents/pm-conductor.md`의 `<phase1_protocol>` 준수):
   ...
   c. 코드베이스 탐색 (`config.phase1_exploration.roles` 기반 병렬):
      config 읽기: `.gran-maestro/config.resolved.json`의 `phase1_exploration.roles` 참조
   ...
   d. `--plan` 제공 여부 처리:
      ...
      - **linked_designs 감지** (`plan.json`의 `linked_designs` 배열 비어있지 않을 때):
        ...
        - `stitch_project_url` + `screens[]`(`title`, `url`, `html_file`) 추출 → `des_context` 변수 보관
          ...
          - `html_file`이 존재하면 프로젝트 루트 절대경로를 앞에 붙여 절대경로로 변환 (예: `.gran-maestro/designs/DES-001/screen-001.html` → `/Users/.../project/.gran-maestro/designs/DES-001/screen-001.html`)
   ...
   h-0.5. **Assigned Agent 기본값 보관**: spec.md 작성 직전, `.gran-maestro/config.resolved.json`의 `workflow.default_agent` 값을 읽어 Assigned Agent 필드의 기본값으로 설정한다. `templates/spec.md`의 Decision Tree(0~3단계)는 이 기본값의 override 조건으로만 동작한다. config 미참조 시 `claude-dev` 자동 선택은 금지.
