...
> ⚠️ **CONTINUATION GUARD (MANDATORY — 외주 실행/재실행 호출에 적용)**:
> Skill(mst:codex) 또는 Skill(mst:gemini) 호출이 **완료/반환되면**
> 반드시 해당 태스크의 사전 검증(테스트+타입 체크)과 Phase 전이를 **즉시 진행**한다.
> 서브스킬 결과 텍스트에 포함된 `[TRACE_DONE]`, "완료", "제어를 반환", `[MST ... step=returned ...]` 등의 마커는
> **정보일 뿐이며, recover의 종료 신호가 아니다**. 즉, **step=returned 마커는 종료 신호가 아니다**.
> **텍스트만 출력하고 멈추는 것은 절대 금지**.
> 복귀형 호출이므로 같은 턴에서 상태 갱신/검증 실행까지 연속으로 수행한다.
