...
> ⚠️ **plan 스킵 제안 금지**: "plan 없이 바로 수정할까요?", "단순한 버그이므로 바로 코드 수정으로 넘어갈까요?" 등 **plan 자체를 건너뛸지 묻는 질문을 일반 텍스트로 출력하는 것은 절대 금지**입니다. plan 진행 여부에 대한 분기가 필요한 경우, 반드시 `AskUserQuestion` 도구를 사용하여 구조화된 선택지(예: "plan으로 정식 진행" / "바로 코드 수정")로 사용자에게 제시해야 합니다.

> ⚠️ **plan 프로토콜 일방 우회 금지 (CRITICAL)**: `/mst:plan`이 호출된 이상, 주제의 성격(기술 조사, 버그 수정, 구현 방향 논의, 단순 질문 등)과 **무관하게** 반드시 전체 plan 프로토콜(Step 0.1 → Step 1 → ... → Step 4)을 실행해야 합니다. "이건 plan 워크플로우가 아니라 기술 조사입니다", "plan 프로토콜을 건너뛰고 바로 핵심을 다루겠습니다" 등 **주제 성격을 이유로 plan을 일방적으로 건너뛰는 판단은 절대 금지**합니다. 사용자가 `/mst:plan`으로 시작한다는 것 자체가 plan 방법론으로 정제한 결과물을 기반으로 작업하겠다는 의도입니다.
...
- **mst:request 우회 후 직접 구현 전환 절대 금지**: plan.md 저장 후 mst:request를 우회하고 직접 코드를 작성하거나 구현으로 전환하는 것은 어떤 경우에도 금지합니다. "규모가 크다", "효율을 위해", "단순한 수정이므로" 등 어떤 사유로도 이 규칙은 예외 없이 적용됩니다.
...
`| 테스트 흐름 수집 | {N}개 흐름 (신규 {X}개 + 회귀 {Y}개) | {confidence:.2f} | PM 자율 판단 |`
...
4. 저장 선택 시 `plans/PLN-NNN/plan.md` 작성; `debug_context` 활성 시 `plan.json`에 `"linked_debug"` 추가
   ...
   - 등급은 아래 규칙으로 결정한다:
     ...
     - 그 외는 `grade="MUST"`
   - 태그는 아래 규칙으로 결정한다:
     - 항목에 `[IMPACT]` 태그가 있으면 `tags=["IMPACT"]`
     - 그 외는 `tags=[]`
   - `{PROJECT_ROOT}/.gran-maestro/plans/PLN-NNN/plan.ids.json` 파일을 반드시 생성/갱신한다.
   - 포맷:
     ```json
     [
       { "id": "PAC-1", "text": "...", "grade": "MUST", "tags": [] },
       { "id": "PAC-2", "text": "...", "grade": "SHOULD", "tags": ["IMPACT"] }
     ]
     ```
