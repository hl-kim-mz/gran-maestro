...
- `AUTO_MODE=false`: 단순 요청이라도 건너뛰지 않는다. **최소 1회 `AskUserQuestion`** 후에만 Step 4로 진행
...
4. `confidence < 0.4`:
   - `Skill(skill: "mst:explore", args: "{관련 탐색 쿼리}")` 선행
   - 탐색 결과 반영 후 confidence 재산정
   - 재산정 confidence `>= 0.4`이면 discussion 실행 후 반영
   - 재산정 confidence `< 0.4`이면 PM이 **가장 안전한 선택**으로 자율 결정
   - `auto-decisions.md`에 즉시 행 추가:
     - `| {항목명} | {결정값} | {confidence:.2f} | explore→discussion 결과 |`
...
Write({PROJECT_ROOT}/.gran-maestro/plans/{PLN_ID}/prompts/plan-draft.tmp, {PLAN_DRAFT 전문})

# 스크립트 실행
python3 {PLUGIN_ROOT}/scripts/mst.py plan render-review \
  --pln {PLN_ID} \
  --plan-draft-file {PROJECT_ROOT}/.gran-maestro/plans/{PLN_ID}/prompts/plan-draft.tmp \
  --qa-summary "{QA_SUMMARY}"
```

스크립트가 `config.plan_review.roles.{role}.enabled == true`인 역할에 대해
`templates/plan-review/{role}.md` 읽기 → 변수 치환 →
`{PROJECT_ROOT}/.gran-maestro/plans/{PLN_ID}/prompts/review-{role}.md` 일괄 생성.
...
- codex/gemini 역할: `TaskOutput(task_id, block: true)` 완료 대기 (Bash background 완료 신호) → `Read({PROJECT_ROOT}/.gran-maestro/plans/PLN-NNN/prompts/review-{role}.log)`로 실제 결과 확인
- claude 역할: `TaskOutput(task_id, block: true)` 반환값을 직접 결과로 사용

**순차 실행 시 (`parallel == false`)**:
- codex/gemini 역할: Skill 호출 자체가 블로킹 완료 → `Read({PROJECT_ROOT}/.gran-maestro/plans/PLN-NNN/prompts/review-{role}.log)`로 결과 확인
- claude 역할: Task 반환값을 직접 결과로 사용 (TaskOutput 불필요)

모든 에이전트 결과를 수집한 후 PM이 분석:
- `NO_ISSUES` 응답: 해당 역할 이슈 없음으로 처리
- `CRITICAL:` 항목: `AUTO_MODE=false`면 사용자 확인 대상, `AUTO_MODE=true`면 가장 안전한 선택 우선으로 자율 처리
- `MAJOR:` 항목: PM이 자체 판단으로 초안에 반영 (`AUTO_MODE=true`에서 필요 시 discussion/explore 병행)
- `MINOR:` 항목: PM이 자체 판단으로 초안에 반영 또는 무시 (`AUTO_MODE=true`면 판단 내역 로그 기록)

#### 3.8.5: 조건부 추가 확인 & 반복 루프
...
- `AUTO_MODE=true`:
  - escalation 질문 없이 모든 escalate 이슈를 `[AUTO_MODE 판단 패턴]`으로 처리
  - CRITICAL 이슈는 confidence가 낮을수록 **가장 안전한 선택**을 우선 적용
  - 각 결정은 즉시 `auto-decisions.md`에 기록하고 PM 초안에 반영
- **반복 판단**:
  - `AUTO_MODE=false`에서 **"반영 없이 진행" 선택 시**: 즉시 Step 4로 진행 (반복 없음)
  - `current_iteration < max_iterations`이고 재리뷰가 필요하면 `current_iteration++` → **3.8.2로 돌아가 재리뷰** (정제된 초안 기준으로 프롬프트 재생성 후 에이전트 재dispatch)
  - `current_iteration >= max_iterations` 또는 재리뷰 불필요 시 Step 4로 진행
...
- MINOR_COUNT > 0 AND MINOR_COUNT >= minor_escalation_threshold → CRITICAL로 취급:
  ...
  - `AUTO_MODE=true`:
    - AskUserQuestion 없이 모든 MINOR 이슈를 `[AUTO_MODE 판단 패턴]`으로 자율 처리
    - 각 결정은 즉시 `auto-decisions.md`에 기록 후 PM 초안 재정제
  - 반복 판단: current_iteration < max_iterations이고 재리뷰 필요 시 3.8.2 / 아니면 Step 4 진행
...
6. `AUTO_MODE=true`이고 plan.md 저장 완료 후 아래 요약을 반드시 출력:
   ...
   - explore→discussion 사용: {AUTO_EXPLORE_DISCUSSION_COUNT}건
