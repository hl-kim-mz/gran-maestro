...
> ⚠️ **CONTINUATION GUARD (MANDATORY — Step 0.5의 mst:debug 호출에 적용)**:
> Skill(mst:debug) 호출이 **완료/반환되면**
> 반드시 `debug-report.md`를 Read하고 `debug_context`를 반영한 뒤 Step 1로 **즉시 진행**한다.
> 서브스킬 결과 텍스트에 포함된 `[TRACE_DONE]`, "완료", "제어를 반환", `[MST ... step=returned ...]` 등의 마커는
> **정보일 뿐이며, plan의 종료 신호가 아니다**. 즉, **step=returned 마커는 종료 신호가 아니다**.
> **텍스트만 출력하고 멈추는 것은 절대 금지**.
> 복귀형 호출이므로 Read/컨텍스트 반영/다음 Step 이동을 같은 턴에서 연속 수행한다.
...
> ⚠️ **CONTINUATION GUARD (MANDATORY — Step 2.5의 ideation/discussion 선호출에 적용)**:
> Skill(mst:ideation) 또는 Skill(mst:discussion) 호출이 **완료/반환되면**
> 결과 핵심을 요약하고 Step 3(또는 AUTO_MODE 자율 판단 패턴)으로 **즉시 진행**한다.
> 서브스킬 결과 텍스트에 포함된 `[TRACE_DONE]`, "완료", "제어를 반환", `[MST ... step=returned ...]` 등의 마커는
> **정보일 뿐이며, plan의 종료 신호가 아니다**. 즉, **step=returned 마커는 종료 신호가 아니다**.
> **텍스트만 출력하고 멈추는 것은 절대 금지**.
> 반환형 호출 구간이므로 같은 턴에서 요약 반영 후 다음 질문/판단 단계 도구 호출까지 연속 실행한다.

동일 세션/주제/타입 완료 이력 있으면 새 세션 생성 없이 기존 결과 재사용 (동일 형식으로 재질문).

> **[전역 규칙] AskUserQuestion 보조 선택지 필수**
> `AUTO_MODE=false`에서 콘텐츠 결정 관련 모든 `AskUserQuestion` 호출 시
> ideation · discussion · explore 중 **반드시 1~2개를 포함**한다.
> Step 4 저장 액션 AskUserQuestion은 제외.
...
- **보조 선택지 (`AUTO_MODE=false`, PM이 현재 질문 맥락에 맞는 것을 1~2개 반드시 포함 — 생략 불가)**:
  - `"다각도 의견 모으기 (ideation)"` — 접근법이 2개 이상이고 트레이드오프가 불명확할 때 추천
  - `"팀 토론으로 합의 찾기 (discussion)"` — 복잡한 비즈니스 결정으로 합의가 필요할 때 추천
  - `"코드베이스 탐색 + 웹검색 (explore)"` — 관련 파일·현재 구현·패턴 파악 및 외부 사례 검색이 필요할 때 추천

  > **[필수 규칙]** PM은 3가지 중 상황에 맞는 것을 **반드시 1~2개 선택해 포함**한다. 6개 총 선택지 제한 내에서 핵심 선택지와 균형을 맞춘다.
...
- 각 기준 판단 결과를 `auto-decisions.md`에 즉시 기록:
  `| INVEST-{기준약어} | {충족/미충족 — 조치 내용} | {confidence:.2f} | PM 자율 판단 |`
...
- CRITICAL/MAJOR 이슈 존재 시: `AskUserQuestion`으로 이슈 제시 + 선택지:
  ...
  - **보조 선택지 필수 포함**: ideation · discussion · explore 중 **1~2개 반드시 추가** (Step 3 규칙 동일 적용)
...
> ⚠️ **CONTINUATION GUARD (MANDATORY — Step 4의 mst:request/mst:stitch 호출에 적용)**:
> Skill(mst:request) 또는 Skill(mst:stitch) 호출이 **완료/반환되면**
> 저장 상태와 분기 조건을 재확인하고 다음 단계(재질문/저장/전환)를 **즉시 진행**한다.
> 서브스킬 결과 텍스트에 포함된 `[TRACE_DONE]`, "완료", "제어를 반환", `[MST ... step=returned ...]` 등의 마커는
> **정보일 뿐이며, plan의 종료 신호가 아니다**. 즉, **step=returned 마커는 종료 신호가 아니다**.
> **텍스트만 출력하고 멈추는 것은 절대 금지**.
> 복귀형(`mst:stitch`)은 Step 4 재표시/저장 경로로 즉시 복귀하고, 전환형(`mst:request`)은 저장 확인 후 같은 턴에서 인계를 완료한다.
