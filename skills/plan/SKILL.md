...
**길이 제한 (필수):** ASCII 도식은 코드 블록 포함 **4줄 이내**로 작성한다.
패널 높이가 고정되어 초과 시 "N lines hidden"으로 잘리므로, 핵심 구조만 담고 나머지는 `description` 필드의 텍스트로 보완한다.
...
3. 최종 승인 시 `.gran-maestro/plans/PLN-NNN/plan.md`에 최초 작성
4. **mst:start 자동 호출 — spec.md 생성 (필수)**
