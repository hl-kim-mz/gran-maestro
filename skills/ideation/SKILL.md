...
> 스킬 실행 초기에 `.gran-maestro/config.json`을 Read하여 `models.claude` 값을 확인하세요.
...
- `provider: "claude"`: `Task(subagent_type: "general-purpose", model: "{config.models.claude}", prompt: "prompts/{roleKey}-prompt.md 파일을 Read하고 지시에 따라 분석. 결과를 opinion-{roleKey}.md에 Write. 완료 후 '완료'")`
