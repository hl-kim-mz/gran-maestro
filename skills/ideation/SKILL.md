...
- `session.json` 업데이트, `context.md`(공유 배경 컨텍스트), 프롬프트 파일(`prompts/{participant.key}-prompt.md`) 작성을
...
1. **단일 응답에서 동시 Write**:
   ...
   - `prompts/{participant.key}-prompt.md` × N — 경량 프롬프트 (공유 파일 참조 + 역할 + 질문만)
   - `prompts/critique-{criticKey}-prompt.md` × M — critic 프롬프트 (아래 템플릿 적용, critics 동적 순회)
