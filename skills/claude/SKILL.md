...
3. 서브에이전트 스폰:
   ...
   - 실행 시간/종료 코드 캡처:
     - Task 시작 직전: `START=$(date +%s%3N)`
     - Task 완료 직후: `END=$(date +%s%3N)`, `DUR=$((END-START))`
     - Task 성공 시 `EXIT=0`, 실패 시 `EXIT=1` (또는 도구가 제공한 종료 코드)

4. `--trace`가 있으면 trace 파일 저장:
   - 파일명 패턴: `claude-{label}-{YYYYMMDD-HHmmss}.md`
   - Task 완료 후 **별도 Bash 한 줄**로 메타데이터-only trace 생성 (Write 도구 호출 금지):
     `Bash("mkdir -p {task_dir}/traces && TS=$(date +%Y%m%d-%H%M%S) && TRACE_FILE={task_dir}/traces/claude-{label}-${TS}.md && { echo '---'; echo 'agent: claude'; echo 'request: {REQ-ID}'; echo 'task: {TASK-NUM}'; echo 'label: {label}'; echo \"timestamp: ${TS}\"; echo \"duration_ms: ${DUR}\"; echo \"exit_code: ${EXIT}\"; echo 'log: requests/{REQ-ID}/tasks/{TASK-NUM}/running.log'; echo '---'; } > \"$TRACE_FILE\"")`
...
5. **결과 반환**
   ...
   **`--trace` 미제공 시**: 서브에이전트 결과만 간결하게 반환한다. 추가 설명, 요약 등 부가 텍스트 출력 절대 금지.
   ...
   > **Exit Code 캡처 (MANDATORY)**: Task/Bash 실행의 종료 코드를 반드시 확인한다.
